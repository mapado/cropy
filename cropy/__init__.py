...
__version__ = '0.1'
__author__ = 'Jerry Nieuviarts, <jerry@mapado.com>'
...
def entropy_crop(img, target_width, target_height, max_steps=10):
    ...
    slice_size = int(round((original_width - target_width) / max_steps))  # calculate slice size based on max steps
    if slice_size == 0:
        slice_size = 1
    ...
    slice_size = int(round((original_height - target_height) / max_steps))  # calculate slice size based on max steps
    if slice_size == 0:
        slice_size = 1
