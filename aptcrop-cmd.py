...
import sys
import os
from aptcrop import entropy_crop
...
usage+= "where [width] and [height] are the requested crop size"
...
if not options.output:
	output = os.path.splitext(options.input_image)[0] +"." + str(crop_width) + "." + str(crop_height) + ".jpg"
else:
	output = options.output

im=io.imread(input_image, as_grey=True) #charge une image au format float
...
region.save(output)
#im_cropped = im[y_offset:crop_height+y_offset, x_offset:crop_width+x_offset]


#io.imsave(output, im_cropped)
